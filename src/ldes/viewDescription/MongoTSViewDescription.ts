...
export class MongoTSViewDescription {
    ...
    public parseViewDescription(store: Store): IViewDescription {
        const parser = new ViewDescriptionParser(
            this.viewIdentifier,
            this.ldesIdentifier,
        );
        return parser.parseViewDescription(
            store,
            this.viewDescriptionIdentifier,
        );
    }

    public generateViewDescription(options: {
        timestampPath: string;
        pageSize?: number;
    }): IViewDescription {
        const ingestorClientType =
            "http://www.example.org/ldes#mongoDBTSIngestor";
        const bucketizationType = "https://w3id.org/ldes#LDESTSFragmentation";

        const bucketStrategy = new BucketizeStrategy(
            this.bucketID(),
            bucketizationType,

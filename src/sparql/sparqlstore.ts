...
export class SparqlStore implements ResourceStore {
    ...
    constructor(
        ...
        const eventStreamSubject = metadataQuads.find(
            (q) =>
                q.predicate.equals(RDF.terms.type) &&
                q.object.equals(LDES.terms.EventStream),
        )!.subject;

        this.timetampPath = metadataQuads.find(
            (q) =>
                q.subject.equals(eventStreamSubject) &&
        ...
        this.metadata = metadataQuads.map((q) => {
            if (q.subject.equals(eventStreamSubject)) {
                return datafactory.quad(
                    datafactory.namedNode(host + path),
                    q.predicate,
                    q.object,
                    q.graph,
                );
    ...
    getRepresentation = async (
        ...
        if (page) {
            ...
            const maxTimeObject = quads
                .filter((q) => q.predicate.value === this.timetampPath)
                .map((x) => x.object)
                .reduce(compareGeneratedAtTime);
            const relationQuads = ldesRelation(
                parseInt(page),
                maxTimeObject.value,
                identifier.path,
                this.relationType,
